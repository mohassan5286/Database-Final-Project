...
function Home({ userId }) {
  ...
  const [name, setUsername] = useState(
    localStorage.getItem("name") || "Muhammad"
  );
  const [email, setEmail] = useState(
    localStorage.getItem("email") || "mohassan5286@gmail.com"
  );
  const [plateNumber, setPlateNumber] = useState(
    localStorage.getItem("plateNumber") || "ABC123"
  );
  ...
  const handleReserve = () => {
    ...
    const lot = parkingLots.find((lot) =>
      lot.spots.some((spot) => spot.spot_id === selectedSpot.spot_id)
    );
    const reservationDetails = {
      ...
      userId: userId,
      parkingSpotId: selectedSpot.spot_id,
    };

    axios
      .post("http://localhost:8081/api/reservations/create", reservationDetails)
      .then((response) => {
        console.log("Reservation successful:", response.data);
        setMessage(
          "Your reservation is successful and all your debt is paid automatically."
        );
  ...
  const handleArrive = (spotId) => {
    axios
      .post("http://localhost:8081/api/reservations/arrived", {
        spotId,
        userId,
      })
      .then((response) => {
        ...
        setMessage(
          "Thank you! You have successfully paid for your reservation."
        );
        setTimeout(() => setMessage(""), 3000);
      })
      .catch((error) => {
    ...
    <>
      {message && (
        <>
          ...
          <div className="arrival-message">{message}</div>
        </>
      )}
        ...
        {filteredParkingLots.map((lot) => (
          <Card key={lot.lot_id}>
            <CardContent>
              ...
              <Button
                onClick={() =>
                  setOpenLotId(openLotId === lot.lot_id ? null : lot.lot_id)
                }
              >
                Show Spots
              </Button>
              {openLotId === lot.lot_id &&
                lot.spots.map((spot) => (
                  <div
                    key={spot.spot_id}
                    className={`spot-card spot-${spot.status.toLowerCase()}`}
                  >
                    <Typography>
                    ...
                    {spot.status === "Available" && (
                      <Button onClick={() => handleClickOpen(spot)}>
                        Reserve
                      </Button>
                    )}
                    {spot.status === "Reserved" && spot.user_id === userId && (
                      <Button onClick={() => handleArrive(spot.spot_id)}>
                        Arrive
                      </Button>
                    )}
                  </div>
